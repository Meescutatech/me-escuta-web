...
/**
 * Overrides do react-day-picker em escopo de MÓDULO.
 *
 * Passá-los como arrow inline dentro do `components={{...}}` criava um *tipo* de
 * componente novo a cada render — o React trata tipo diferente como outro
 * componente e DESMONTA/REMONTA a árvore inteira do calendário (~120 nós) a cada
 * clique num atalho de período. Em escopo de módulo a identidade é estável.
 */
const defaultClassNames = getDefaultClassNames();

const CalendarRoot: NonNullable<
...
const CalendarChevron: NonNullable<
  React.ComponentProps<typeof DayPicker>["components"]
...
const CalendarWeekNumber: NonNullable<
  React.ComponentProps<typeof DayPicker>["components"]
...
function Calendar({
  ...
  // `DayButton` fecha sobre `locale`, então o mapa é memoizado em vez de içado.
  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        today: cn(
          // Quando hoje está selecionado, some com o fundo cinza da célula:
          // senão o quadrado do `bg-muted` vaza atrás do pill arredondado do
          // botão (cantos "cortados"). O botão cuida do realce (primary/range).
          "rounded-[var(--cell-radius)] bg-muted text-foreground data-[selected=true]:bg-transparent",

...
/**
 * shadcn/ui `chart` (https://ui.shadcn.com/docs/components/chart), copiado em 10/09/2026 para o
 * dashboard do dono (W-D4) sobre recharts 2.x. O que muda em relação ao original: nada de
 * estrutural — só o comentário. As cores das séries entram pelo `config` (`color` ou `theme`) e
 * viram `--color-<chave>` no container; os gráficos usam `var(--color-<chave>)`.
 */
                ...
                <>
                  ...
                  <div className={cn("flex flex-1 justify-between leading-none", nestLabel ? "items-end" : "items-center")}>
                    ...
                    {item.value !== undefined && item.value !== null && (
                      <span className="font-medium text-foreground tabular-nums">{typeof item.value === "number" ? item.value.toLocaleString("pt-BR") : String(item.value)}</span>
                    )}
                  </div>
                </>
              )}

...
function semAtividade(r: LinhaEquipe): boolean {
  return (r.conversas.atual ?? 0) === 0 && (r.tarefasConcluidas.atual ?? 0) === 0 && (r.tarefasCriadas.atual ?? 0) === 0 && (r.ganhos?.vendas ?? 0) === 0 && (r.cargaAgora ?? 0) === 0;
}

export function TabelaPessoas({ dados, className }: { dados: DadosDashboardDono; className?: string }) {
  const todas = dados.equipe.filter((r) => r.ativo || r.temAtividade);
  const buscando = dados.filtros.q.length > 0;
  const comAtividade = todas.filter((r) => buscando || !semAtividade(r));
  const recolhidas = buscando ? [] : todas.filter(semAtividade);
  const linhas = comAtividade.filter((r) => casaBusca(r.nome, dados.filtros.q));
  ...
  const semLeitura = [!temGanhos && "ganhos por pessoa", !temCarga && "carga agora"].filter(Boolean).join(", ");

  return (
    <Bloco denso className={className}>
          ...
          <table className="w-full min-w-[640px] border-collapse">
            <thead>
              <tr className="border-y border-border bg-muted/40">
                ...
                <Th>Conversas</Th>
                <Th title="mediana até a 1ª resposta; a barra compara com a mediana do próprio tipo (agente/pessoa)">1ª resposta</Th>
                <Th title="concluídas no período">Tarefas</Th>
                <Th title="vendas ganhas no período com esta pessoa como dona">Ganhos</Th>
                <Th title="conversas abertas com esta pessoa agora" className="pr-3.5">Carga</Th>
              </tr>
            </thead>
            <tbody>
              <tr className="border-b border-border font-semibold">
                ...
                <Td>{fmtInt(concl)}</Td>
                <Td>{temGanhos ? <NumPct n={fmtMoeda(valor)} pct={`${fmtInt(vendas)}`} tom="verde" /> : "—"}</Td>
              ...
              {linhas.map((r) => {
                ...
                return (
                  <tr key={r.ator} className="border-b border-border/70 last:border-0 hover:bg-muted/30">
                    ...
                    <Td className="font-semibold">{fmtInt(c)}</Td>
                    <Td className={cn("font-semibold", lento && "text-danger-ink")} title={r.primeiraResposta.amostra > 0 ? `mediana de ${r.primeiraResposta.amostra} conversas · mediana ${r.tipo === "agente" ? "dos agentes" : "das pessoas"}: ${fmtMinutos(mediaTipo)}` : undefined}>
                    ...
                    <Td>
                      {(r.tarefasConcluidas.atual ?? 0) === 0 && (r.tarefasCriadas.atual ?? 0) > 0 ? (
                        <NumPct n="0" pct={`${fmtInt(r.tarefasCriadas.atual)} criadas`} />
                      ) : (
                        <span className="font-semibold">{fmtInt(r.tarefasConcluidas.atual)}</span>
                      )}
                    </Td>
              ...
              {linhas.length === 0 && (
                <tr>
                  <td colSpan={6}><Vazio texto={`Ninguém com "${dados.filtros.q}" no período.`} /></td>
                </tr>
              )}
              {recolhidas.length > 0 && (
                <tr>
                  <td colSpan={6} className="px-3.5 py-1.5 text-[11.5px] text-muted-foreground">
                    +{fmtInt(recolhidas.length)} sem atividade no período: {recolhidas.map((r) => r.nome).join(", ")}
                  </td>
                </tr>
              )}
            </tbody>
...
export function TabelaNumeros({ dados, className }: { dados: DadosDashboardDono; className?: string }) {
      ...
      <div className="mt-2 overflow-x-auto">
        <table className="w-full min-w-[380px] border-collapse">
          ...
          <tbody>
            ...
            {linhas.map((c) => {
              ...
              return (
                <tr key={c.canal_id} className="border-b border-border/70 last:border-0 hover:bg-muted/30">
                  ...
                  <Td><NumPct n={fmtInt(r)} pct={rec > 0 ? fmtPct(r / rec) : null} /></Td>
            ...
            {linhas.length === 0 && (
              <tr>
                <td colSpan={4}><Vazio texto={`Nenhum número com "${dados.filtros.q}".`} /></td>
              </tr>
            )}
          </tbody>
        </table>
...
export function TabelaFunil({ dados, className }: { dados: DadosDashboardDono; className?: string }) {
  ...
  const Linha = ({ e }: { e: EtapaResumo & { tempoMedioDias: number | null } }) => {
    ...
    return (
      <tr className="border-b border-border/70 last:border-0 hover:bg-muted/30">
        ...
        <Td className="pr-3.5 font-semibold">{e.tipo === "aberto" ? fmtInt(e.estoque) : ""}</Td>
      </tr>
    );
  };
          ...
          <table className="w-full min-w-[440px] border-collapse">
            <thead>
              <tr className="border-y border-border bg-muted/40">
                ...
                <Th className="pr-3.5" title="leads na etapa agora">Agora</Th>
              </tr>
            </thead>
            <tbody>
              <tr className="border-b border-border font-semibold">
                ...
                <Td />
                <Td className="pr-3.5"><NumPct n={fmtInt(estoque)} pct={`${fmtMoeda(dados.valorNegociacao.total)} em negociação`} /></Td>
              </tr>
              {linhas.map((e) => (
              ...
              {linhas.length === 0 && (
                <tr>
                  <td colSpan={5}><Vazio texto={`Nenhuma etapa com "${dados.filtros.q}".`} /></td>
                </tr>
              )}
            </tbody>
          </table>
